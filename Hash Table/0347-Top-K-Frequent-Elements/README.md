...
## 📊 Complexity Analysis
* **Time Complexity:** 𝙊(正式𝗡) — Building the hash map takes $O(N)$. Populating the buckets takes $O(U)$ where $U$ is the number of unique numbers ($U \le N$). Collecting the final array reads exactly $K$ values. The overall runtime is strictly linear.
